...
class edrone():
    ...
    def __init__(self):
        ...
        self.goal_point = [19.0,72.0, 8.440994388]

        # self.set_point[2]+=5.00
        ...
        self.Ki = [0.0, 0.0, 0.0*0.008]
        self.Kd = [0.3*18000*873, 2102*0.3*18000, 4476*0.3]
        ...
        self.del_lati = 0.000013552
        self.del_longi = 0.000014245

        self.a1_lati = 18.9999864489
        self.a1_longi = 71.9999430161
        self.a1_alti = 8.44099749139
        self.box_points = {"A1":[self.a1_lati,self.a1_longi,self.a1_alti],"B1":[self.a1_lati+self.del_lati,self.a1_longi,self.a1_alti], "C1":[self.a1_lati+2*self.del_lati,self.a1_longi,self.a1_alti],
                              "A2":[self.a1_lati,self.a1_longi+self.del_longi,self.a1_alti],"B2":[self.a1_lati+self.del_lati,self.a1_longi+self.del_longi,self.a1_alti],"C2":[self.a1_lati+2*self.del_lati,self.a1_longi+self.del_longi,self.a1_alti],
                              "A3":[self.a1_lati,self.a1_longi+2*self.del_longi,self.a1_alti],"B3":[self.a1_lati+self.del_lati,self.a1_longi+2*self.del_longi,self.a1_alti],"C3":[self.a1_lati+2*self.del_lati,self.a1_longi+2*self.del_longi,self.a1_alti]}
        self.marker_des ={}
        self.marker_names=[]
        ...
        with open("/home/dhruvi/Desktop/catkin_ws/src/vitarana_drone/scripts/manifest.csv","r") as file:
            ...
            for row in reader:
                self.marker_des[str(row[0])]=[float(row[1]),float(row[2]),float(row[3])]
                self.marker_names.append(str(row[0]))
            file.close()  
        # print(self.marker_dest)   
        # print(self.marker_names)

        self.loc=[self.box_points[self.marker_names[0]],self.marker_des[self.marker_names[0]],self.box_points[self.marker_names[1]],self.marker_des[self.marker_names[1]],self.box_points[self.marker_names[2]],self.marker_des[self.marker_names[2]],self.initial_point]
    ...
    def detection_clbk(self,msg):
        ...
        if self.isDetected:self.detect=self.isDetected
    #--------------------------------------------------------------------------------------------------------------------------
    ...
    def distance(self):
        self.dist = math.sqrt(math.pow(110692.0702932625 * (self.goal_point[0] - self.curr_point[0]) , 2) + math.pow(105292.0089353767 * (self.goal_point[1] - self.curr_point[1]) , 2))
        if 0<self.dist < 3 :
            self.t = self.dist*120
        elif 3 <= self.dist < 10 :
            self.t = self.dist*50
        ...
        self.dx = (self.goal_point[0] - self.curr_point[0])/self.t
        self.dy = (self.goal_point[1] - self.curr_point[1])/self.t
    ...
    def handler(self):
        ...
        if self.drone_state==0: #change to takeoff -> state=1
            ...
            if abs(self.error[2])<0.15:
                self.prev_setpoint[0],self.prev_setpoint[1],self.prev_setpoint[2] = self.set_point[0],self.set_point[1],self.set_point[2]
                self.set_point[2]=max(self.set_point[2],float(self.loc[self.loc_count][2])+self.fly_hieght)
                self.goal_point[0],self.goal_point[1],self.goal_point[2]=self.loc[self.loc_count][0],self.loc[self.loc_count][1],self.loc[self.loc_count][2]
                self.distance()                
                self.change(2)
        elif self.drone_state==2:
            if self.isLoaded == True :
                if self.detect:
                    if not self.isDetected:
                        ...
                        if -0.2 < self.err_x_m < 0.2 and -0.2 < self.err_y_m < 0.2 :
                            ...
                            self.isLoaded=False
                            self.loc_count+=1
                            self.set_point[2]=max(self.set_point[2],self.loc[self.loc_count][2]+self.fly_hieght)
                            self.goal_point[0],self.goal_point[1],self.goal_point[2]=self.loc[self.loc_count][0],self.loc[self.loc_count][1],self.loc[self.loc_count][2]
                            self.detect = False
                            self.stopDetection=False
                            self.drone_state=1
                    ...
                    self.altitude_pub.publish(self.loc[self.loc_count][2])
                    if(abs(self.set_point[2]-self.curr_point[2])<0.1 ):
                        self.detect = True                   
                else:
                    self.distance()
                ...
                if abs(self.goal_point[0]-self.curr_point[0])<0.0000800 and abs(self.goal_point[1]-self.curr_point[1])<0.0000800:
                    ...
                    if abs(self.goal_point[0]-self.curr_point[0])<0.000001200 and abs(self.goal_point[1]-self.curr_point[1])<0.000001200:
                        self.change(3)
                        self.loc_count+=1
                else:
                    self.distance()
    ...
    def check_pick_gripper(self):
        ...
        while not res.result:
            ...
            if res.result:self.isLoaded=True
    ...
    def wall_foll_waypoint_gen(self):
        if 1<self.obs_dist[self.regions["front"]]<5 and 1<self.obs_dist[self.regions["back"]]<5 and self.obs_dist[self.regions["right"]]>5 and self.obs_dist[self.regions["left"]]>5:
        ...
        elif 1<self.obs_dist[self.regions["right"]]<5 and self.obs_dist[self.regions["front"]]>5 and self.obs_dist[self.regions["back"]]>5 and self.obs_dist[self.regions["left"]]>5:
            self.check = True
            self.sensor = "R"
            if abs(self.dx) > abs(self.dy) :
                self.set_point[0]=self.prev_setpoint[0]#-self.dx/6
            ...
            rospy.loginfo("   R   ")
        elif self.obs_dist[self.regions["right"]]>5 and self.obs_dist[self.regions["front"]]>5 and self.obs_dist[self.regions["back"]]>5 and 1<self.obs_dist[self.regions["left"]]<5:
            self.check = True
            self.sensor = "L"
            if abs(self.dx) > abs(self.dy) :
                self.set_point[0]=self.prev_setpoint[0]#-self.dx/6
