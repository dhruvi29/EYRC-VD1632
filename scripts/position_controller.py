...
class edrone():
    ...
    def __init__(self,set_point):
        ...
        rospy.Subscriber('/pid_tuning_pitch', PidTune, self.latitude_set_pid) #temporary
        rospy.Subscriber('/pid_tuning_roll', PidTune, self.longitude_set_pid) #temporary
    ...
    def pid(self):#"""self,Kp=[0,0,2778.0*0.06],Kd=[0,0,1301.0*0.3],Ki=[0,0,4.0*0.008]"""):
        ...
        # rospy.loginfo()
        rospy.loginfo("pErr of latitude:%f",self.pErr[0])
        # if self.output[2] > 1510: self.output[2] = 1510
        # if self.output[2] < 1490: self.output[2] = 1490
        # if self.output[1] > 1510: self.output[1] = 1510
        # if self.output[1] < 1490: self.output[1] = 1490
        # if self.output[0] > 1510: self.output[0] = 1510
        # if self.output[0] < 1490: self.output[0] = 1490

        # updating output throttle(thrust)
        self.attitude_cmd.rcThrottle = 1500+self.output[2]
        self.attitude_cmd.rcPitch=1500+self.output[1]
        self.attitude_cmd.rcRoll=1500+self.output[0]
        # rospy.loginfo("rcthrottle:%f,output throttle:%f",self.attitude_cmd.rcThrottle,self.output[2])
        # rospy.loginfo("rcpitch:%f,rcroll:%f",self.attitude_cmd.rcPitch,self.attitude_cmd.rcRoll)
        # updating error
        ...
        self.longitude_error_pub.publish(self.error[1]*1000000)
    ...
    while not rospy.is_shutdown():
        if flag[0]==0:
            ...
            if abs(e_drone_1.error[2])<0.08 :
                ...
                if i==30:
                    flag[0]=1
                    e_drone_2.error=e_drone_1.error
                    e_drone_2.prev_error=e_drone_1.prev_error
                    e_drone_2.iterm=e_drone_1.iterm
                    e_drone_2.pErr=e_drone_1.pErr
            else:
            ...
            if abs(e_drone_3.error[2])<=0.001:
                e_drone_3.attitude_cmd.rcThrottle=1000
                e_drone_3.attitude_cmd.rcRoll=1500
                e_drone_3.attitude_cmd.rcPitch=1500
                e_drone_3.attitude_cmd.rcYaw=1500
                e_drone_3.attitude_cmd.drone_cmd_pub.publish(e_drone_3.attitude_cmd)
                print("ninu")    
