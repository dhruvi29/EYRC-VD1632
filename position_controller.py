...
class edrone():
	...
	def __init__(self):
		...
		self.altitude_range_Kp = 1
		self.altitude_range_Kd = 1
		self.altitude_range_Ki = 1
	...
	def pid(self):
		...
		self.altitude_range_output=self.altitude_range_pErr +(self.altitude_range_Ki*self.altitude_range_iterm)*self.sample_time + self.altitude_range_Kd*self.altitude_range_dErr
		
		#if self.output[0]>1024: self.output[0]=1024
		#if self.output[1]>1024: self.output[1]=1024
		#if self.output[2]>1024: self.output[2]=1024
		
		#if self.output[0]<0: self.output[0]=0
		#if self.output[1]<0: self.output[1]=0
		#if self.output[2]<0: self.output[2]=0
		
		#updating output throttle(thrust)
		self.attitude_cmd.rcThrottle = self.altitude_range_output
		#updating error
		...
		self.altitude_error_pub.publish(self.altitude_range_error)
