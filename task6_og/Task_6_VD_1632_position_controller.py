...
class edrone():
    ...
    def distance(self):
        ...
        #     self.no_points = self.dist*1.1
        # elif 140<= self.dist < 160:
        #     self.no_points = self.dist*1.8   
        # elif 160 <= self.dist < 200 :
        #     self.no_points = self.dist*0.9 
        else :
            self.t = self.dist*1.2 #self.dist*0.81
        if self.t == 0:
            self.t = 1           
    ...
    def box_dropping(self): 
        ...
        if self.ret == False:
            ...
            if abs(self.goal_point[0]-self.curr_point[0])<0.00020000 and abs(self.goal_point[1]-self.curr_point[1])<0.00020000:
                self.set_point[0]=self.goal_point[0]
                self.set_point[1]=self.goal_point[1]
    ...
    def handler(self):
        ...
        if self.drone_state==0:#change to takeoff -> state=1
            ...
            if self.gripper == "True":
                ...
                if abs(self.goal_point[0]-self.curr_point[0])<0.0001700 and abs(self.goal_point[1]-self.curr_point[1])<0.0001700:
                    self.set_point[0]=self.goal_point[0]
                    self.set_point[1]=self.goal_point[1]
